...
-------------------------------------------------------------------------
-- Cascada para SCL
Reg_SCL_1: entity work.RegistroTipoD port map(clk => clk, D => to_X01(scl),    Q => scl_d1);
...
-- Cascada para SDA
Reg_SDA_1: entity work.RegistroTipoD port map(clk => clk, D => to_X01(sda),    Q => sda_d1);
    ...
    if reset = '1' then
        ...
        if start_detected = '1' then
            ...
            case state is
                ...
                when ADDRESS =>
                    -- 1. Leemos el bit y restamos el contador EN LA SUBIDA
...
--                            bit_count <= bit_count - 1; 
                    ...
                    -- 2. En la bajada, si ya leímos 8 bits, comprobamos y enviamos ACK
                    if scl_falling = '1' then
                        if bit_count = 0 then
                            ...
                            -- Si no hemos llegado a 0, restamos el contador aquí
                            bit_count <= bit_count - 1;
                        end if;
                    end if;

                when ACK_ADDRESS =>
                    -- El maestro está leyendo nuestro ACK. En la bajada soltamos el bus.
                    if scl_falling = '1' then
                        bit_count <= 7;
                        if rw_bit = '0' then
                            state <= RECEIVE_DATA;
                            sda_out <= '1'; -- Soltamos bus para que hable el maestro
                        else
                            state <= TRANSMIT_DATA;
                            read_req <= '1'; 
                            shift_reg <= data_in;
                            sda_out <= data_in(7); -- Sacamos el bit 7 inmediatamente
                        end if;
                    end if;
...
--                        end if;
                    ...
                    if scl_falling = '1' then
                        if bit_count = 0 then
                            data_out <= shift_reg;
                            data_valid <= '1'; 
                            sda_out <= '0'; -- Enviamos ACK para el dato recibido
                            state <= ACK_DATA;
								else
                            bit_count <= bit_count - 1;
                        end if;
                ...
                when TRANSMIT_DATA =>
                    if scl_falling = '1' then
                        if bit_count = 0 then
                            ...
                            bit_count <= bit_count - 1;
                            sda_out <= shift_reg(bit_count - 1); -- Sacamos el siguiente bit
                        end if;
                    end if;
